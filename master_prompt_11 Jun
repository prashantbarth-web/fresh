...
**CATEGORY SLUG RULE:** Take "Category Name" from Column 2 of `master_list.csv` → convert to lowercase hyphenated slug.
...
### Source:
`master_list.csv` has all articles with slug, title, tags and status.
`all_done.csv` has list of all published article,  use them for interlinking only.
